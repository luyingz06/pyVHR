...
class Pipeline():
    ...
    def run_on_video(self, frames, 
        ...
        # skip skin extractor for now because it's preprocessed
        # 1. set skin extractor
        # target_device = 'GPU' if cuda else 'CPU'
        # if roi_method == 'convexhull':
        #     sig_processing.set_skin_extractor(SkinExtractionConvexHull(target_device))
...
class DeepPipeline(Pipeline):
    ...
    def run_on_video(self, frames, cuda=True, method='MTTS_CAN', bpm_type='welch', post_filt=False, verb=True, crop_face=False):
        """ 
        Runs the pipeline on a specific video file.
        ...
        if bpm_type == 'welch':
            if cuda:
                bpmES = BVP_to_BPM_cuda(bvps, fps, minHz=DeepPipeline.minHz, maxHz=DeepPipeline.maxHz)
            else:
                bpmES = BVP_to_BPM(bvps, fps, minHz=DeepPipeline.minHz, maxHz=DeepPipeline.maxHz)
        else:
            raise ValueError("The only 'bpm_type' supported for deep models is 'welch'")
           
        # median BPM from multiple estimators BPM
        median_bpmES = BPM_median(bpmES)
