...
class SkinExtractionHSV:
    ...
    def __init__(self, device='GPU'):
        self.device = device
        self.lower_threshold = torch.tensor([1, 48, 20]).reshape((3, 1, 1))
        self.upper_threshold = torch.tensor([20, 255, 255]).reshape((3, 1, 1))

    def extract_skin_(self, image: torch.Tensor, ldmks) -> Tuple[torch.Tensor, torch.Tensor]:
        # quick and dirty inRange
        img = image.copy()
        img = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)

        image = torch.tensor(img, dtype=torch.float32).permute(2, 0, 1)
        mask = image * (image >= self.lower_threshold)
        mask = mask * (mask <= self.upper_threshold)

        mask = mask.to(torch.uint8)
        image_uint8 = image.to(torch.uint8)

        # gaussian blur with torchvision
        mask = F.gaussian_blur(mask.float(), [3,3]).to(torch.uint8)
        
        # bitwise_and
        skin_image = torch.bitwise_and(image_uint8, mask)

        # skin_image.permute(2, 0, 1)[2, 1, 0].permute(1, 2, 0)
        skin_image = skin_image.permute(1, 2, 0)
        
        #conver back to rgb
        skin_image = cv2.cvtColor(skin_image.numpy(), cv2.COLOR_HSV2RGB)

        rows = torch.any(skin_image, dim=2)
        cols = torch.any(skin_image, dim=1)

        rmin, rmax = torch.nonzero(rows, as_tuple=True)[0][[0, -1]]
        cmin, cmax = torch.nonzero(cols, as_tuple=True)[0][[0, -1]]
        ...
        plt.savefig('/home/luying/pyVHR/pyVHR/videos/cropped_im.png')
    ...
    def extract_skin(self, image, ldmks):
        ...
        # if use test.py, should add this line to conver from BGR to RGB 
        # both needed!!!!!!
